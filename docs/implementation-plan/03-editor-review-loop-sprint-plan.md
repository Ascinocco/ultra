# Milestone 3 Sprint Plan: Sandbox Terminal Workflow
...
Deliver a real sandbox-aware terminal loop so a user can move from a thread into the correct checkout, sync `.env`, run tests from an integrated terminal, and request changes or approve the work without leaving the chat workspace.

## Definition of Done

Milestone 3 is done when:

- a review-ready thread can make its sandbox the active testing context
...
### Sprint 1: Sandbox Context Model

Goal:

Make sandbox contexts real and selectable.

Tasks:

- implement sandbox context persistence
- implement `sandboxes.list`
- implement `sandboxes.get_active`
- implement `sandboxes.set_active`
- add sandbox slices to frontend store
- build shell sandbox selector and metadata row

Exit criteria:

- active sandbox persists per project
- selector switches cleanly between main checkout and thread sandboxes
...
- `.env` is copied into the active sandbox
...
- ensure terminal launches with sandbox cwd
- implement `terminal.run_saved_command`
- add default commands such as `test`, `dev`, `lint`, `build`
- show command output and session state in the terminal drawer

Exit criteria:

- terminals always open in the active sandbox
- saved commands always respect the active sandbox
...
- build `Request Changes` and `Approve` actions in the chat and thread workflow
...
- handle missing-sandbox and missing-runtime-file errors
- test project switching while sandbox selection persists
...
- user can trust what sandbox they are operating in

## Suggested Work Order

Recommended order:

1. sandbox persistence and store
2. shell sandbox selector
...
Do not wire approval UI before sandbox selection and terminal targeting work correctly.

## Deliverables by Layer

### Frontend

- sandbox selector and metadata
- terminal drawer
...
- can I select a review-ready thread sandbox?
- does the correct checkout become active?
- is `.env` present in the sandbox after activation?
...
The product should show sync status instead of silently mutating sandbox state.
...
At the end of Milestone 3, a user should be able to plan in Chat, create a thread, select its sandbox, test it in the right terminal context, and drive the thread back for more work or complete it.
